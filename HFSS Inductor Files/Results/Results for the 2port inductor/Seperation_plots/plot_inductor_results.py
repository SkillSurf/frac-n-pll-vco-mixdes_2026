...
def extract_radius_from_column(col_name):
    """Extract radius value from column name"""
    import re
...
def plot_Q_vs_freq(df, output_file='Q_vs_frequency.pdf'):
    ...
    # Get frequency column and filter to 10 GHz
    freq_mask = df['F [GHz]'] <= 10
    freq = df.loc[freq_mask, 'F [GHz]']
    
    # Plot Q for each radius
    ...
    for idx, col in enumerate(q_columns):
        radius = extract_radius_from_column(col)
        label = f'S = {radius} μm' if radius else col
        ax.plot(freq, df.loc[freq_mask, col], label=label, color=colors[idx], marker='', linewidth=1.8)
    ...
    ax.legend(loc='upper left', bbox_to_anchor=(1.02, 1), frameon=True, fancybox=True, shadow=False, framealpha=0.9)
...
def plot_combined(df, output_file='combined_Q_L_vs_frequency.pdf'):
    ...
    for idx, col in enumerate(q_columns):
        radius = extract_radius_from_column(col)
        label = f'S = {radius} μm' if radius else col
        ax1.plot(freq, df.loc[freq_mask, col], label=label, color=colors_q[idx], linewidth=1.8)
    ...
    ax1.legend(loc='upper left', bbox_to_anchor=(1.02, 1), frameon=True, fancybox=True, shadow=False, framealpha=0.9)
    ...
    for idx, col in enumerate(l_columns):
        radius = extract_radius_from_column(col)
        label = f'S = {radius} μm' if radius else col
        ax2.plot(freq, df.loc[freq_mask, col], label=label, color=colors_l[idx], linewidth=1.8)
    ...
    ax2.legend(loc='upper left', bbox_to_anchor=(1.02, 1), frameon=True, fancybox=True, shadow=False, framealpha=0.9)
...
def plot_dual_axis(df, output_file='dual_axis_L_Q_vs_frequency.pdf'):
    ...
    for idx, col in enumerate(l_columns):
        radius = extract_radius_from_column(col)
        label = f'$L_{{diff}}$: S = {radius} μm' if radius else col
        line, = ax1.plot(freq, df.loc[freq_mask, col], label=label, color=colors_l[idx], 
    ...
    for idx, col in enumerate(q_columns):
        radius = extract_radius_from_column(col)
        label = f'$Q_{{diff}}$: S = {radius} μm' if radius else col
        line, = ax2.plot(freq, df.loc[freq_mask, col], label=label, color=colors_q[idx], 
                         linewidth=1.8, linestyle='--')
        lines2.append(line)
    ...
    ax1.legend(lines, labels, loc='upper left', frameon=True, fancybox=True, 
               shadow=False, framealpha=0.95, ncol=1, fontsize=8.5, bbox_to_anchor=(1.15, 1))
...
def main():
    ...
    csv_file = 'Seperation_variations.csv'
