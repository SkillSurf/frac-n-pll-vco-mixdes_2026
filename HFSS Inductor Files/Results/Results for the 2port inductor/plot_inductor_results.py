...
def plot_Q_vs_freq(df, output_file='Q_vs_frequency.pdf'):
    ...
    for idx, col in enumerate(q_columns):
        ...
        label = f'$R_{{out}}$ = {radius} μm' if radius else col
...
def plot_combined(df, output_file='combined_Q_L_vs_frequency.pdf'):
    ...
    ax1.legend(loc='best', frameon=True, fancybox=True, shadow=False, framealpha=0.9, ncol=2)
    ...
    ax2.legend(loc='best', frameon=True, fancybox=True, shadow=False, framealpha=0.9, ncol=2)
...
def plot_dual_axis(df, output_file='dual_axis_L_Q_vs_frequency.pdf'):
    ...
    for idx, col in enumerate(l_columns):
        radius = extract_radius_from_column(col)
        label = f'$L_{{diff}}$: $R_{{out}}$ = {radius} μm' if radius else col
    ...
    for idx, col in enumerate(q_columns):
        radius = extract_radius_from_column(col)
        label = f'$Q_{{diff}}$: $R_{{out}}$ = {radius} μm' if radius else col
    ...
    ax1.legend(lines, labels, loc='upper left', frameon=True, fancybox=True, 
               shadow=False, framealpha=0.95, ncol=1, fontsize=8.5, bbox_to_anchor=(0.01, 0.99))
...
def main():
    ...
    csv_file = 'Radius_180um_220um.csv'
